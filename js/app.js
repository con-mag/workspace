(() => {
  ...
  let currentRoute='home', openClient=null, currentPath='', selectedFile=null, folderCache=new Map(), clientMetaCache=new Map();
  ...
  function fileIcon(kind,folder=false){
    if(folder)return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7.5A2.5 2.5 0 0 1 5.5 5H10l2 2h6.5A2.5 2.5 0 0 1 21 9.5v7A2.5 2.5 0 0 1 18.5 19h-13A2.5 2.5 0 0 1 3 16.5z"/><path d="M3.5 9h17"/></svg>';
    if(kind==='نص')return 'T'; if(kind==='Word')return 'W'; if(kind==='Excel')return '▦'; if(kind==='PDF')return 'P'; if(kind==='صورة')return '◫'; if(kind==='فيديو')return '▶'; return '□'
  }
  ...
  async function getFolderEntries(clientId,path=''){
    ...
    const out=[...set.values()].sort((a,b)=>a.type===b.type?a.name.localeCompare(b.name,'ar'):a.type==='folder'?-1:1);
    folderCache.set(key,out);return out;
  }
  ...
  async function renderClient(id){
    ...
    $$('[data-entry-path]',$('#fileGrid')).forEach(card=>card.onclick=e=>{if(e.target.closest('[data-delete-path]'))return;const p=card.dataset.entryPath,t=card.dataset.entryType;t==='folder'?goClient(id,p):goClient(id,currentPath,p)});
    $$('[data-client-root]',$('#clientPage')).forEach(b=>b.onclick=()=>goClient(id,''));$$('[data-client-path]',$('#clientPage')).forEach(b=>b.onclick=()=>goClient(id,b.dataset.clientPath));$$('[data-delete-path]',$('#fileGrid')).forEach(b=>b.onclick=e=>{e.stopPropagation();deleteClientPath(id,b.dataset.deletePath)});
    if(selectedFile) await renderSelectedFile(id,selectedFile);
  }
  async function renderSelectedFile(clientId,path){
    ...
    if(hasCloud()){
      ...
      if(f){
        ...
        viewer.innerHTML=`<div class="viewer-head"><div><h2>${esc(name)}</h2><p>${esc(path)}</p></div><span class="tag">${esc(kind)}</span></div><div class="viewer-body">
          ...
          <div class="viewer-actions">${textable?'<button class="primary-btn" id="saveFileBtn">حفظ التعديلات</button>':''}<button class="link-btn" id="openFileBtn">فتح / تنزيل</button><button class="danger-btn" id="deleteFileBtn">حذف الملف</button></div><div class="path-line">${esc(f.path||path)}</div></div>`;
        if(textable)$('#saveFileBtn').onclick=()=>saveTextFile(clientId,path,f);
        $('#openFileBtn').onclick=()=>window.open(f.download_url||f.html_url,'_blank','noopener');
    ...
    if(local&&local.type==='file'){
      ...
      viewer.innerHTML=`<div class="viewer-head"><div><h2>${esc(name)}</h2><p>${esc(path)}</p></div><span class="tag">محلي · ${esc(kind)}</span></div><div class="viewer-body">
        ...
        <div class="viewer-actions">${textable?'<button class="primary-btn" id="saveFileBtn">حفظ التعديلات</button>':''}<button class="link-btn" id="openFileBtn">فتح / تنزيل</button><button class="danger-btn" id="deleteFileBtn">حذف الملف</button></div><div class="path-line">${esc(path)}</div></div>`;
      if(textable)$('#saveFileBtn').onclick=()=>saveLocalTextFile(clientId,path,local);
      $('#openFileBtn').onclick=()=>{const a=document.createElement('a');a.href=url;a.download=name;a.target='_blank';a.click();setTimeout(()=>URL.revokeObjectURL(url),1500)};
      $('#deleteFileBtn').onclick=()=>deleteClientPath(clientId,path);return;
    }

    const localUrl=`clients/${encodeURIComponent('ريتاج علي')}/${path.split('/').map(encodeURIComponent).join('/')}`;
    viewer.innerHTML=`<div class="viewer-head"><div><h2>${esc(name)}</h2><p>${esc(path)}</p></div><span class="tag">${esc(kind)}</span></div><div class="viewer-body"><div class="viewer-note">هذا ملف من القالب الأصلي للمشروع. يمكنك إنشاء نسخة قابلة للتحرير أو رفع نسخة جديدة من الزر «＋ ملف».</div><div class="viewer-actions"><a class="link-btn" href="${localUrl}" target="_blank" rel="noopener">فتح الملف</a>${textable?'<button class="primary-btn" id="createRemoteCopy">إنشاء نسخة قابلة للتحرير</button>':''}</div><div class="path-line">${esc(path)}</div></div>`;
